...
*   **Bug**: The root `.env.example` lists `CHROMA_SERVER_HOST` and `CHROMA_SERVER_HTTP_PORT`. However, Pydantic Settings in [config.py](file:///d:/cognitive-oos/backend/app/core/config.py#L21-L22) looks for `CHROMA_HOST` and `CHROMA_PORT`.
...
*   **Environment Variables required on Vercel**:
    *   `NEXT_PUBLIC_API_URL`: Points to your hosted FastAPI backend (e.g., `https://cognitive-os-backend.railway.app`).
