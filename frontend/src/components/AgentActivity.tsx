"use client";

import { Activity, Code, Search, CheckCircle, Bot } from "lucide-react";
import { useChatStore } from "@/store";

export function AgentActivity() {
  const { agents } = useChatStore();

  const getIcon = (id: string) => {
    switch (id) {
      case "supervisor":
        return Bot;
      case "coder-agent":
        return Code;
      case "research-agent":
        return Search;
      default:
        return Activity;
    }
  };

  const isSystemActive = agents.some((a) => a.status === "executing");

  return (
    <div className="glass-panel rounded-2xl p-5 border border-border flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Activity className="w-5 h-5 text-foreground" />
          <h2 className="font-semibold text-sm text-foreground">Assistant Activity</h2>
        </div>
        <span className={`flex items-center gap-2 text-xs px-2.5 py-1 rounded-full border transition-all ${
          isSystemActive
            ? "bg-foreground/10 text-foreground border-foreground/20"
            : "bg-muted text-muted-foreground border-border"
        }`}>
          <span className="relative flex h-1.5 w-1.5">
            {isSystemActive ? (
              <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-foreground" />
            ) : (
              <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-muted-foreground/60" />
            )}
          </span>
          {isSystemActive ? "Processing" : "Live"}
        </span>
      </div>

      <div className="space-y-4">
        {agents.map((agent) => {
          const Icon = getIcon(agent.id);
          const isExecuting = agent.status === "executing";
          
          return (
            <div key={agent.id} className="relative p-4 bg-card/60 dark:bg-card/40 border border-border rounded-xl overflow-hidden transition-all duration-300 hover:bg-muted/40">
              {isExecuting && (
                <div className="absolute top-0 left-0 w-full h-[1.5px] bg-gradient-to-r from-transparent via-primary to-transparent animate-[shimmer_3s_infinite]" />
              )}
              <div className="flex items-center gap-3 mb-2">
                <Icon className={`w-4 h-4 ${agent.color}`} />
                <span className="font-medium text-xs text-foreground/90">{agent.name}</span>
                <span className={`ml-auto text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded border ${
                  isExecuting ? "bg-foreground/10 text-foreground border-foreground/20 dark:bg-primary/10 dark:text-primary dark:border-primary/20" : "bg-muted text-muted-foreground/60 border-transparent"
                }`}>
                  {agent.status}
                </span>
              </div>
              <p className="text-[11px] text-muted-foreground flex items-center gap-2 font-light">
                {isExecuting ? (
                  <span className="w-3 h-3 border-2 border-primary/30 border-t-primary rounded-full animate-spin shrink-0" />
                ) : (
                  <CheckCircle className="w-3 h-3 text-muted-foreground/40 shrink-0" />
                )}
                <span className="truncate">{agent.task}</span>
              </p>
            </div>
          );
        })}
      </div>
    </div>
  );
}
